#!/usr/bin/env python3
"""Validate ByFTP documentation links, index coverage and version neutrality."""
...
def check_link(source: Path, raw: str) -> None:
    ...
    if not target.exists():
        fail(f"missing local link target: {source.relative_to(ROOT)} -> {raw}")


def main() -> int:
    files = markdown_files()
    if not files:
        fail("no Markdown documents found")
    ...
    for path in files:
        ...
        if path.parent == DOCS and VERSIONED_DOC_TITLE_RE.search(text):
            fail(f"documentation page has a stale versioned title: {path.relative_to(ROOT)}")
