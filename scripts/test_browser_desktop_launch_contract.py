...
class BrowserDesktopLaunchContractTests(unittest.TestCase):
    ...
    def test_desktop_parser_installer_and_uninstall_share_contract(self) -> None:
        parser = read(ROOT / "cmd" / "ghostftp" / "launch_target.go")
        ...
        desktop = read(ROOT / "internal" / "desktop" / "startup_target.go")

        self.assertIn('strings.EqualFold(u.Scheme, "ghostftp")', parser)
        self.assertIn('strings.EqualFold(u.Host, "connect")', parser)
