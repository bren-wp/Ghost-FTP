...
def main() -> int:
    ...
    for browser in BROWSERS:
        marker = f"Ghost-FTP-${{VERSION}}-{browser}-Extension.zip"
        if marker not in release:
            fail(f"release.yml missing {marker}")
    ...
    for retired in ("macos:", "MACOS_", "macOS", "macos/"):
        if retired in no_key:
            fail(f"release-no-key.yml still references retired macOS surface: {retired}")
    for marker in ("Ghost-FTP-${version}-Setup.exe", "Ghost-FTP-${version}-Portable.exe", "Ghost-FTP-${version}-Android.apk"):
        if marker not in no_key:
