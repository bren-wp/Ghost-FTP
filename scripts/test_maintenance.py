...
class MaintenanceRegressionTests(unittest.TestCase):
    ...
    def test_ios_preset_replacement_does_not_delete_last_good_value_first(self) -> None:
    ...
    def test_web_version_and_fail_closed_boundaries(self) -> None:
        ...
        self.assertIn("Putanja ne smije sadržavati backslash", paths)
        self.assertIn("Putanja ne smije sadržavati dvostruke separatore", paths)
        self.assertIn("Komponente . i .. nisu dopuštene", paths)
        self.assertIn("ctype_digit", profiles)
        self.assertNotIn("trim((string)($input['host']", profiles)
        self.assertIn("Host ne smije imati rubne razmake", host_guard)
