...
class MaintenanceRegressionTests(unittest.TestCase):
    ...
    def test_auxiliary_audits_report_only_validated_scope(self) -> None:
        ...
        self.assertIn("DESKTOP_SURFACE_AUDIT_SCOPE=WINDOWS,LINUX,ANDROID,RETIRED_SURFACES", desktop)
        self.assertIn("ANDROID_SOURCE_SURFACE=ACTIVE", desktop)
        self.assertIn("MACOS_SOURCE_SURFACE=RETIRED", desktop)
    ...
    def test_version_history_and_current_release_contract(self) -> None:
        ...
        self.assertIn("Last actually published GitHub Release: **0.0.8**", readme)
        self.assertIn("13 platform artifacts / 16 public files", readme)
        self.assertIn(f"Current source version: **{version}**", docs_index)
        self.assertIn("Last actually published GitHub Release: **0.0.8**", docs_index)
