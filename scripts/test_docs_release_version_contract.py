...
class ReleaseDocumentationContractTests(unittest.TestCase):
    ...
    def test_release_verification_tracks_published_version(self):
    ...
    def test_release_facing_docs_match_006_shape(self):
        ...
        for relative in (
            ...
            self.assertNotIn("18 platform artifacts / 21 public files", text, relative)
    ...
    def test_published_history_is_current(self):
        for relative in (
            "README.md",
            "docs/README.md",
            "docs/INSTALLATION.md",
            ...
            self.assertIn("0.0.6", text, relative)
            self.assertNotIn("last actually published github release: **0.0.5**", text, relative)
            self.assertNotIn("last actually published github release remains **0.0.5**", text, relative)
