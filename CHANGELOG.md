...
- Added the matching AppKit master rail and embedded engine-backed transfer queue on macOS while preserving the existing typed shared-engine boundary; the no-secret 0.0.8 distribution publishes the universal ad-hoc validation bundle without claiming Developer ID signing or notarization.
