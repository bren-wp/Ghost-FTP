...
require_text "release signing secret" "$ROOT/.github/workflows/ghostftp-android-release.yml" 'GHOSTFTP_ANDROID_KEYSTORE_B64'
require_text "release signature verification" "$ROOT/.github/workflows/ghostftp-android-release.yml" 'verify --verbose --print-certs'
require_text "release signing cleanup" "$ROOT/.github/workflows/ghostftp-android-release.yml" 'rm -f android/release.keystore android/signing.properties'
require_absent "unsigned production APK fallback" "$ROOT/.github/workflows/ghostftp-android-release.yml" 'app-release-unsigned.apk'
