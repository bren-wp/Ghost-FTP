...
The Files surface may place local and remote panes side-by-side only when there is enough width. On narrower layouts they stack inside the Files surface rather than mixing unrelated Connections, Transfer Queue, Bookmarks, Settings, Connection info or About controls into the same scroll flow.
...
Connection info is a real runtime-owned, privacy-safe diagnostic surface. It reports only the current connection state, active protocol, security mode and current transfer state. It deliberately excludes the server host, username, password, private keys and saved server paths.

For FTPS it reports that certificate and hostname verification are enabled. For plain FTP it explicitly identifies the connection as unencrypted compatibility mode. When disconnected it shows no protocol identity and no active connection state.
