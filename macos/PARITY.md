...
Directory Compare is wired to the shared `Engine.CompareDirectoryItems` contract using fresh Local/Remote listings from the existing engine. The native comparison window shows conservative status for both sides and enables Open Both only when `Engine.SynchronizedDirectoryName` proves an ordinary directory exists on both sides. Compare is cancellable before queued disconnect, generation/path-bound, and reuses the normal Local/Remote navigation paths; it performs no independent filesystem or protocol traversal.
