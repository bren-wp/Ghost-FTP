...
About is the only Mac application surface that exposes publisher/author identity, sourced from the dedicated macOS About-only metadata file. Product version remains bound to root `VERSION` by the native build, while product/support identity remains explicit. Connection info is intentionally compact and privacy-safe: it exposes only connected state and protocol plus the no-telemetry/local-profile statement. It deliberately excludes server identity, visible remote paths, raw logs, protected secrets and generic diagnostic payloads.
