...
export function QuickConnectionDialog({ prefill, onClose }: Props) {
  ...
  const disconnectProfile = useConnections((s) => s.disconnect);
  ...
  const canConnect = host.trim().length > 0 && (protocol === "ftp" || username.trim().length > 0);
  ...
  const testConnection = async () => {
    ...
    try {
      await connectTemporary(profile);
      const session = useConnections.getState().sessions.find((s) => s.profileId === profile.id);
      if (session) await disconnectProfile(session.sessionId);
      setTestStatus("ok");
    } catch {
      const session = useConnections.getState().sessions.find((s) => s.profileId === profile.id);
      if (session) await disconnectProfile(session.sessionId).catch(() => {});
      setTestStatus("error");
    }
  };
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" className="ghost-new-connection-dialog anim-modal flex h-[min(628px,calc(100dvh-24px))] w-[min(752px,94vw)] flex-col overflow-hidden rounded-xl border border-accent/70 bg-[#061a2d] shadow-[0_0_0_1px_rgba(65,181,255,.08),0_30px_90px_rgba(0,0,0,.7),0_0_38px_rgba(31,149,255,.15)]" onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <div className="grid grid-cols-[1.1fr_1.7fr_.55fr] gap-3">
            ...
            <Field label="Port"><input type="number" value={port} onChange={(e)=>setPort(Number(e.target.value)||PROTOCOL_DEFAULT_PORT[protocol])}/></Field>
          </div>
