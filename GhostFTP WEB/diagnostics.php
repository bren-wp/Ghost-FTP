...
$expectedDiagnosticsPath = rtrim(GhostFTP_url('diagnostics'), '/');
$rewriteReady = rtrim($requestPath, '/') === $expectedDiagnosticsPath;
$noIndexReady = is_file(GhostFTP_ROOT . '/robots.txt');
...
$checks = [
    ...
    ['storage/ zapisiv', is_writable(GhostFTP_STORAGE) ? 'Da' : 'Ne', is_writable(GhostFTP_STORAGE), 'storage'],
    ['storage/tmp zapisiv', is_writable(GhostFTP_STORAGE . '/tmp') ? 'Da' : 'Ne', is_writable(GhostFTP_STORAGE . '/tmp'), 'storage'],
    ...
    ['Privatni/local server targeti', GhostFTP_private_hosts_allowed() ? 'Dopušteni administratorskom postavkom' : 'Blokirani (preporučeno za shared hosting)', !GhostFTP_private_hosts_allowed(), 'security'],
    ...
    ['session.gc_maxlifetime', (string)ini_get('session.gc_maxlifetime') . ' s', (int)ini_get('session.gc_maxlifetime') >= GhostFTP_session_limits()['idle'], 'limits'],
    ['HTTPS', $isHttps ? 'Aktivan' : 'Nije detektiran', $isHttps, 'security'],
];
$requiredOk = version_compare(PHP_VERSION, '8.1.0', '>=') && extension_loaded('ftp') && (function_exists('sodium_crypto_secretbox') || function_exists('openssl_encrypt')) && is_writable(GhostFTP_STORAGE) && is_writable(GhostFTP_STORAGE . '/tmp');
?><!doctype html>
<html lang="hr">
<head>
    <?php $pageTitle = 'Dijagnostika · ' . GhostFTP_app_name(); require __DIR__ . '/app/Views/head.php'; ?>
    </head>
...
<main id="main" class="auth-card diagnostics-card auth-card-premium">
    ...
    <div class="diagnostic-list">
        <?php foreach ($checks as [$name, $value, $ok, $group]): ?>
            <div class="diagnostic-row" data-group="<?= GhostFTP_e((string)$group) ?>">
                <span><strong><?= GhostFTP_e((string)$name) ?></strong><small><?= GhostFTP_e((string)$value) ?></small></span>
