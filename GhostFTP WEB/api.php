...
if (!Auth::check()) {
    GhostFTP_json(['ok' => false, 'error' => 'Sesija je istekla.'], 401);
}
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    GhostFTP_json(['ok' => false, 'error' => 'Method not allowed.'], 405);
}
$contentLength = (int)($_SERVER['CONTENT_LENGTH'] ?? 0);
$postMax = GhostFTP_ini_bytes(ini_get('post_max_size'));
if ($contentLength > 0 && $postMax < PHP_INT_MAX && $contentLength > $postMax && $_POST === [] && $_FILES === []) {
    GhostFTP_json(['ok' => false, 'error' => 'Zahtjev prelazi PHP post_max_size limit ovog hostinga.'], 413);
}

$csrf = $_SERVER['HTTP_X_CSRF_TOKEN'] ?? $_POST['csrf'] ?? null;
if (!GhostFTP_verify_csrf(is_string($csrf) ? $csrf : null)) {
    GhostFTP_json(['ok' => false, 'error' => 'Sigurnosni token nije valjan.'], 419);
}
...
try {
    if ($action === 'me') {
        GhostFTP_json(['ok' => true, 'user' => Auth::user(), 'preferences' => $preferences->clientState()]);
    }
    if ($action === 'save_preferences') {
        ...
        GhostFTP_json(['ok' => true, 'preferences' => $preferences->saveClientState($payload)]);
    }
    if ($action === 'profiles') {
        GhostFTP_json(['ok' => true, 'profiles' => $store->all(false)]);
    }
    if ($action === 'save_profile') {
        $profile = $store->save($_POST);
        AppLogger::event('profile.save', ['profile_id' => $profile['id'] ?? '']);
        GhostFTP_json(['ok' => true, 'profile' => $profile]);
    }
    if ($action === 'delete_profile') {
        $id = (string)($_POST['id'] ?? '');
        $store->delete($id);
        AppLogger::event('profile.delete', ['profile_id' => $id]);
        GhostFTP_json(['ok' => true]);
    }
    ...
    if ($action === 'test_profile_draft') {
        ...
        GhostFTP_json(['ok' => true, 'message' => 'Veza i početna putanja su dostupne.', 'elapsed_ms' => $elapsedMs]);
    }
    if ($action === 'ping') {
        GhostFTP_json(['ok' => true, 'version' => GhostFTP_VERSION, 'time' => gmdate('c')]);
    }
    ...
    if ($action === 'favorites') {
        GhostFTP_json(['ok' => true, 'favorites' => $preferences->favorites($profileId)]);
    }
    if ($action === 'toggle_favorite') {
        $path = PathGuard::normalizeRelative((string)($_POST['path'] ?? '/'));
        GhostFTP_json(['ok' => true, 'favorites' => $preferences->toggleFavorite($profileId, $path)]);
    }
    ...
    switch ($action) {
        case 'test':
            ...
            GhostFTP_json(['ok' => true, 'message' => 'Veza je uspješna.']);

        case 'list':
            $path = PathGuard::normalizeRelative((string)($_POST['path'] ?? '/'));
            GhostFTP_json(['ok'=>true,'path'=>$path,'items'=>$client->list($path),'favorites'=>$preferences->favorites($profileId),'features'=>['zip'=>class_exists(ZipArchive::class),'checksum'=>true,'batch_rename'=>true,'analyze'=>true]]);
        ...
        case 'mkdir':
            ...
            GhostFTP_json(['ok'=>true,'path'=>$target]);
        ...
        case 'move':
            ...
            if ($action === 'move' && $source === $destination) GhostFTP_json(['ok'=>true,'path'=>$source,'skipped'=>true]);
            if ($source === $destination && $action === 'copy') {
                $destination = $ops->uniquePath($destination);
            } elseif ($ops->exists($destination)) {
                if ($policy === 'skip') GhostFTP_json(['ok'=>true,'path'=>$destination,'skipped'=>true]);
            ...
            GhostFTP_json(['ok'=>true,'path'=>$destination,'skipped'=>false]);

        case 'duplicate':
            $source = PathGuard::ensureNotRoot((string)($_POST['source'] ?? ''));
            $path = $ops->duplicate($source);
            AppLogger::event('item.duplicate', ['profile_id'=>$profileId,'source'=>$source,'destination'=>$path]);
            GhostFTP_json(['ok'=>true,'path'=>$path]);

        case 'read':
            $path = PathGuard::ensureNotRoot((string)($_POST['path'] ?? ''));
            $content = $client->read($path, 4194304);
            GhostFTP_json(['ok'=>true,'content'=>$content,'etag'=>hash('sha256',$content),'bytes'=>strlen($content)]);
        ...
        case 'write':
            ...
            GhostFTP_json(['ok'=>true,'bytes'=>strlen($content),'etag'=>hash('sha256',$content)]);

        case 'checksum':
            $path = PathGuard::ensureNotRoot((string)($_POST['path'] ?? ''));
            GhostFTP_json(['ok'=>true] + $ops->checksum($path, (string)($_POST['algorithm'] ?? 'sha256')));

        case 'analyze':
            $path = PathGuard::normalizeRelative((string)($_POST['path'] ?? '/'));
            GhostFTP_json(['ok'=>true,'stats'=>$ops->analyze($path)]);

        case 'batch_rename':
            $items = GhostFTP_json_array((string)($_POST['items'] ?? '[]'), 200);
            $result = $ops->batchRename($items, GhostFTP_truncate((string)($_POST['find'] ?? ''),120), GhostFTP_truncate((string)($_POST['replace'] ?? ''),120), GhostFTP_truncate((string)($_POST['prefix'] ?? ''),80), GhostFTP_truncate((string)($_POST['suffix'] ?? ''),80));
            AppLogger::event('item.batch_rename', ['profile_id'=>$profileId,'count'=>$result['renamed'] ?? 0]);
            GhostFTP_json(['ok'=>true] + $result);
        ...
        case 'search':
            ...
            GhostFTP_json(['ok'=>true,'results'=>$ops->search($root, GhostFTP_truncate(trim((string)($_POST['query'] ?? '')),100))]);
        ...
        case 'zip':
            ...
            GhostFTP_json(['ok'=>true,'path'=>$destination]);
