...
const pageLanguage = document.querySelector('meta[name="ghostftp-language"]')?.content || 'en';
...
const state = {
    ...
    language: pageLanguage, translations: readMetaJson('ghostftp-i18n'), preferences: {},
};
...
function browserLocale() {
    if (state.language === 'no') return 'nb';
    if (state.language === 'zh') return 'zh-Hans';
...
function queuePreferencePatch(patch) {
    state.preferences = { ...state.preferences, ...patch, language: state.language };
    window.clearTimeout(preferenceSaveTimer);
    preferenceSaveTimer = window.setTimeout(() => {
        api('save_preferences', { preferences: JSON.stringify(state.preferences) }).catch(() => undefined);
    }, 250);
}
...
async function loadPreferences() {
    ...
    state.language = typeof preferences.language === 'string' && preferences.language ? preferences.language : pageLanguage;
    if (state.language !== pageLanguage) {
        window.location.reload();
        return false;
    }
    document.documentElement.lang = browserLocale();
    state.showHidden = preferences.showHidden !== false;
    state.uploadConflict = ['overwrite', 'rename', 'skip'].includes(preferences.uploadConflict) ? preferences.uploadConflict : 'rename';
    if (preferences.sort && ['name', 'size', 'modified'].includes(preferences.sort.key)) {
        state.sort = { key: preferences.sort.key, direction: Number(preferences.sort.direction) < 0 ? -1 : 1 };
    }
    el.showHidden.checked = state.showHidden;
    el.uploadConflict.value = state.uploadConflict;
    return true;
}
...
async function loadProfiles() {
    ...
    renderProfiles();
    if (!state.profileId && state.profiles.length) {
        const preferredId = typeof state.preferences.lastProfile === 'string' ? state.preferences.lastProfile : '';
        const preferred = state.profiles.find((profile) => profile.id === preferredId) || state.profiles[0];
...
function renderProfiles() {
    ...
    if (!state.profiles.length) {
        el.profiles.innerHTML = `<span class="muted tiny">${escapeHtml(tr('profile.none', 'No saved connections yet.'))}</span>`;
        return;
    }
    for (const profile of state.profiles) {
        const row = document.createElement('div');
        row.className = `profile-item${profile.id === state.profileId ? ' active' : ''}`;
        row.innerHTML = `<span class="profile-dot"></span><button type="button" class="profile-main"><span class="profile-copy"><strong>${escapeHtml(profile.label)}</strong><small>${escapeHtml(profile.protocol.toUpperCase())} · ${escapeHtml(profile.host)}</small></span></button><button class="profile-edit" type="button" aria-label="${escapeHtml(tr('profile.edit', 'Edit connection'))}">⋯</button>`;
...
async function selectProfile(id) {
    ...
    queuePreferencePatch({ lastProfile: id });
    renderProfiles();
    closeSidebar();
    const profile = currentProfile();
    el.connectionName.textContent = profile?.label || tr('profile.server', 'Server');
...
async function openPath(path) {
    ...
    setStatus(tr('connection.connecting', 'Connecting…'));
    try {
        ...
        state.selected.clear();
        renderAll();
        setStatus(tr('status.connected', 'Connected'), true);
    } catch (error) {
        setStatus(tr('status.connection_error', 'Connection error'), false);
...
function visibleItems() {
    ...
    const query = (el.filter.value || '').trim().toLocaleLowerCase(locale);
    return state.items.filter((item) => {
        ...
        else result = String(a.modified || '').localeCompare(String(b.modified || ''), locale);
...
function renderFiles() {
    ...
    for (const item of items) {
        ...
        row.dataset.path = path; row.dataset.type = item.type;
        if (state.selected.has(path)) row.classList.add('selected');
        row.innerHTML = `<td><input class="row-check" type="checkbox" ${state.selected.has(path) ? 'checked' : ''} aria-label="${escapeHtml(tf('files.select', 'Select {name}', { name: item.name }))}"></td><td><div class="file-name"><span class="file-icon">${fileIcon(item)}</span><strong title="${escapeHtml(item.name)}">${escapeHtml(item.name)}</strong></div></td><td class="desktop-only">${item.type === 'dir' ? escapeHtml(tr('files.folder', 'Folder')) : escapeHtml(tr('files.file', 'File'))}</td><td>${item.type === 'dir' ? '—' : formatBytes(item.size)}</td><td class="desktop-only">${escapeHtml(formatDate(item.modified))}</td><td><div class="row-actions"><button class="row-open" type="button" title="${escapeHtml(tr('common.open', 'Open'))}">↗</button><button class="row-more" type="button" title="${escapeHtml(tr('common.more', 'More'))}">⋯</button></div></td>`;
        row.querySelector('.row-check').addEventListener('change', (event) => toggleSelection(path, event.target.checked));
        row.querySelector('.row-open').addEventListener('click', () => openItem(item));
        row.querySelector('.row-more').addEventListener('click', (event) => showContext(event.currentTarget, item));
        row.addEventListener('dblclick', () => openItem(item));
        row.addEventListener('contextmenu', (event) => { event.preventDefault(); showContextAt(event.clientX, event.clientY, item); });
        el.rows.append(row);
    }
    el.empty.classList.toggle('hidden', items.length !== 0);
    el.itemSummary.textContent = tf('files.summary', '{shown} shown · {total} total', { shown: items.length, total: state.items.length });
...
function renderFavorites() {
    ...
    if (!state.favorites.length) {
        el.favorites.innerHTML = `<span class="muted tiny">${escapeHtml(tr('favorites.none', 'No favorites for this connection.'))}</span>`;
        return;
    }
...
function updateSelection() {
    ...
    el.selectionCount.textContent = tf('selection.count', '{count} selected', { count });
}
...
async function saveEditor() {
    try {
        ...
        state.editorEtag = result.etag || '';
        toast(tr('editor.saved', 'File saved.'));
...
function updateEditorCursor() {
    ...
    $('#editorCursor').textContent = tf('editor.cursor', 'Line {line}, column {column}', { line: lines.length, column: lines.at(-1).length + 1 });
}
...
async function contextAction(action) {
    ...
    if (action === 'duplicate') return mutate('duplicate', { source:path }, tr('item.duplicated', 'Item duplicated.'));
    if (action === 'delete') return deleteItems([item]);
    if (action === 'rename') {
        const name = await promptValue(tr('common.rename', 'Rename'), tr('item.new_name', 'New name'), item.name); if (!name) return;
        return mutate('rename', { from:path, name }, tr('item.renamed', 'Item renamed.'));
    }
    if (action === 'chmod') {
        const mode = await promptValue(tr('permissions.title', 'Permissions'), tr('permissions.mode', 'CHMOD (for example 644 or 755)'), item.permissions || (item.type === 'dir' ? '755' : '644')); if (!mode) return;
        return mutate('chmod', { path, mode }, tr('permissions.updated', 'Permissions updated.'));
    }
    if (action === 'copy' || action === 'move') {
        const destination = await promptValue(action === 'copy' ? tr('copy.to', 'Copy to…') : tr('move.to', 'Move to…'), tr('destination.path', 'Destination path'), joinPath(state.path,item.name)); if (!destination) return;
        return mutate(action, { source:path, destination, conflict:'rename' }, action === 'copy' ? tr('copy.done', 'Copy completed.') : tr('move.done', 'Move completed.'));
    }
    if (action === 'extract') return mutate('extract', { path, destination:state.path }, tr('zip.extracted', 'ZIP archive extracted.'));
}
...
async function deleteItems(items) {
    if (!items.length || !confirm(tf('delete.confirm_selected', 'Delete {count} selected items? This action may be irreversible.', { count: items.length }))) return;
    try {
        if (items.length === 1) {
        ...
        state.selected.clear(); toast(tr('delete.done', 'Deletion completed.')); await openPath(state.path);
    } catch (error) { toast(error.message,true); }
}
...
async function previewImage(path, name) {
    ...
    } catch (error) { toast(error.message || tr('preview.failed', 'Preview failed.'),true); }
}
...
async function newFolder() {
    const name = await promptValue(tr('folder.new', 'New folder'), tr('folder.name', 'Folder name'));
    if (name) mutate('mkdir',{path:state.path,name},tr('folder.created', 'Folder created.'));
}

async function newFile() {
    const name = await promptValue(tr('file.new', 'New file'), tr('file.name', 'File name'));
    if (name) {
        await mutate('new_file',{path:state.path,name,content:''},tr('file.created', 'File created.'));
        openEditor(joinPath(state.path,name));
    }
}
...
async function searchRemote() {
    ...
    openModal(el.searchModal); el.searchStatus.textContent=tr('search.searching', 'Searching…'); el.searchResults.innerHTML='';
    try {
        const result=await api('search',{profile_id:state.profileId,path:state.path,query}); const rows=result.results||[]; el.searchStatus.textContent=tf('search.results', '{count} results', { count: rows.length });
...
function showProfileModal(profile=null) {
    ...
    el.profileTitle.textContent=profile?tr('profile.edit_server', 'Edit server'):tr('profile.new_server', 'New server'); $('#deleteProfileBtn').classList.toggle('hidden',!profile); $('#duplicateProfileBtn').classList.toggle('hidden',!profile); updateProfileFields(); openModal(el.profileModal);
}
...
async function saveProfile(event){event.preventDefault();try{const result=await api('save_profile',profilePayload());closeModal(el.profileModal);toast(tr('profile.saved', 'Connection saved.'));await loadProfiles();if(result.profile?.id)await selectProfile(result.profile.id);}catch(error){toast(error.message,true);}}
...
async function deleteProfile(){const id=el.profileForm.elements.id.value;if(!id||!confirm(tr('profile.delete_confirm', 'Delete this saved connection?')))return;try{await api('delete_profile',{id});if(state.profileId===id){state.profileId='';state.items=[];}closeModal(el.profileModal);await loadProfiles();toast(tr('profile.deleted', 'Connection deleted.'));}catch(error){toast(error.message,true);}}
async function duplicateProfile(){const id=el.profileForm.elements.id.value;if(!id)return;try{await api('duplicate_profile',{id});closeModal(el.profileModal);await loadProfiles();toast(tr('profile.duplicated', 'Connection duplicated.'));}catch(error){toast(error.message,true);}}

async function uploadFiles(files, folder=false){if(!state.profileId||!files.length)return;const list=[...files];for(const file of list){if(Number.isFinite(uploadMax)&&file.size>uploadMax){toast(tf('upload.php_limit', '{name}: file exceeds the PHP upload limit.', { name: file.name }),true);return;}}el.transferPanel.classList.remove('hidden');el.cancelUpload.classList.remove('hidden');for(let i=0;i<list.length;i++){const file=list[i];el.transferTitle.textContent=`Upload ${i+1}/${list.length}`;el.transferDetail.textContent=file.name;el.transferProgress.value=0;const relative=folder?(file.webkitRelativePath||file.name):'';const task=uploadRequest({profileId:state.profileId,path:state.path,file,relativePath:relative,conflict:state.uploadConflict,onProgress:(percent,loaded,total)=>{el.transferProgress.value=percent;el.transferDetail.textContent=`${file.name} · ${percent}% · ${formatBytes(loaded)} / ${formatBytes(total)}`;}});state.activeUpload=task.xhr;try{await task.promise;}catch(error){if(error.name!=='AbortError')toast(error.message,true);break;}finally{state.activeUpload=null;}}el.transferPanel.classList.add('hidden');el.cancelUpload.classList.add('hidden');await openPath(state.path);}

function bulkDestination(action){return promptValue(action==='copy'?tr('copy.selected', 'Copy selected'):tr('move.selected', 'Move selected'),tr('destination.folder', 'Destination folder'),state.path,tr('destination.keep_names', 'Each item keeps its name. Conflicts are renamed automatically.'));}
async function bulkCopyMove(action){const items=selectedItems();if(!items.length)return;const root=await bulkDestination(action);if(!root)return;try{for(const item of items){const source=joinPath(state.path,item.name);const destination=joinPath(root,item.name);await api(action,{profile_id:state.profileId,source,destination,conflict:'rename'});}state.selected.clear();toast(action==='copy'?tr('copy.done', 'Copy completed.'):tr('move.done', 'Move completed.'));await openPath(state.path);}catch(error){toast(error.message,true);}}
async function bulkZip(){const paths=[...state.selected];if(!paths.length)return;const name=await promptValue(tr('zip.create', 'Create ZIP'),tr('zip.name', 'Archive name'),'archive.zip');if(!name)return;await mutate('zip',{paths:JSON.stringify(paths),destination:joinPath(state.path,name.endsWith('.zip')?name:`${name}.zip`)},tr('zip.created', 'ZIP archive created.'));}
function bulkDownload(){const paths=[...state.selected];if(paths.length)submitDownload(endpoints.archive,{profile_id:state.profileId,paths:JSON.stringify(paths),name:'Ghost-FTP-download.zip'});}
...
$('#upBtn').addEventListener('click',()=>openPath(parentPath(state.path)));$('#refreshBtn').addEventListener('click',()=>openPath(state.path));el.favoriteBtn.addEventListener('click',toggleFavorite);el.filter.addEventListener('input',renderFiles);el.showHidden.addEventListener('change',()=>{state.showHidden=el.showHidden.checked;queuePreferencePatch({showHidden:state.showHidden});renderFiles();});el.uploadConflict.addEventListener('change',()=>{state.uploadConflict=el.uploadConflict.value;queuePreferencePatch({uploadConflict:state.uploadConflict});});el.selectAll.addEventListener('change',()=>{for(const item of visibleItems()){const path=joinPath(state.path,item.name);el.selectAll.checked?state.selected.add(path):state.selected.delete(path);}renderFiles();updateSelection();});$$('.sort-button').forEach(button=>button.addEventListener('click',()=>{const key=button.dataset.sort;if(state.sort.key===key)state.sort.direction*=-1;else state.sort={key,direction:1};queuePreferencePatch({sort:{...state.sort}});renderFiles();}));
...
bootstrapApp().catch((error)=>toast(error.message,true));
