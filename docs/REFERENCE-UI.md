...
7. persists the maintained repository images only when the capture commit is still the eligible branch head.

The current repository images predate the final 1.1.7 Settings/About capture set and remain historical evidence only until the exact 1.1.7 release head passes the authentic screenshot workflow. They must not be treated as proof for changed 1.1.7 dialog geometry.

Historical Site Manager navigation evidence was regenerated by authentic UI workflow run **#122** from source commit `3de83bad46420ac896d8c7bf0073d66c9547c81c` and persisted by screenshot commit `f4e7cf73136fab885fd270b1552ede6ced224e66`:
...
These historical hashes remain useful for provenance but do not satisfy the final 1.1.7 UI gate. A future UI change must regenerate authentic evidence instead of retaining stale screenshot hashes.
