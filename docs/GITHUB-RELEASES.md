...
Ghost FTP **0.0.6** is the active release candidate. The last actually published GitHub Release is **0.0.5** until the protected 0.0.6 publication workflow completes successfully.
...
For 0.0.6 the branch is `release/ghostftp-v0.0.6`. The branch trigger accepts it only when its semantic version equals root `VERSION` and its SHA equals exact current `main`.
...
Ghost FTP 0.0.6 publishes **13 platform artifacts / 16 public files**.
...
macOS remains an active development/source surface and is not in the public 0.0.6 allow-list until real Developer ID signing and Apple notarization are proven.
...
It signs `Ghost-FTP-0.0.6-Android.apk`, runs `apksigner verify --verbose --print-certs`, normalizes the signer certificate SHA-256 digest and requires exact equality with `GHOSTFTP_ANDROID_CERT_SHA256`. The workflow never generates a production Android publisher identity.
...
Chrome, Edge, Firefox and Opera packages are rebuilt deterministically from the 0.0.6 source tree under `extensions/`. Official manifests remain zero-permission/zero-host-permission and publication does not add a supported browser-to-desktop handoff, remote service or automatic update mechanism.
