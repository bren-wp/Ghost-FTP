...
The 0.0.8 queue contract is protected by:
...
Authentic Windows/Linux/Android runtime evidence remains the exact-head 18-image evidence bundle; macOS development validation is separate and is not silently counted as public notarization evidence.

## 0.0.8 release boundary

Root `VERSION` is **0.0.8**. Queue priority is part of the maintained desktop/source contract; publication still requires exact-head tests/builds, exact post-merge main verification and canonical `ghostftp-v0.0.8` publication/readback/retention.
