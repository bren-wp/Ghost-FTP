...
10. `Ghost FTP RC11 release` publishes only after the current main commit passes quality, real FTP/FTPS/SFTP E2E, and successful native Windows/Linux builds.
11. Generate/upload SHA-256 checksums with the release assets.
12. Retain older releases.

## Required RC11 assets

Release publication must not proceed if the real protocol E2E workflow or either native platform build fails.
