    ...
    <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby={titleId} className="ghost-properties-dialog anim-modal flex flex-col overflow-hidden rounded-xl border border-accent/70 bg-[#061a2d] shadow-elev-3" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        {tab === "general" ? <>
          ...
          <div className="mt-4 border-t border-border pt-3"><div className="mb-2 text-[12px] font-semibold text-text-muted">Sync Options</div><CheckRow label="Apply permissions recursively to all files and folders" checked={recursive} onChange={setRecursive} disabled={!isDir || !fs.chmodRecursive}/><CheckRow label="Preserve timestamps during transfer — backend policy not yet available" checked={false} onChange={()=>{}} disabled/><CheckRow label="Include in synchronization — managed by Sync configuration" checked={false} onChange={()=>{}} disabled/><div className="mt-1 text-[10px] leading-4 text-text-dim">Unavailable policies are intentionally disabled instead of presenting controls that do not change native behavior.</div></div>
