...
Current Ghost FTP version: **1.0.3**
...
Ghost FTP releases use `ghostftp-vX.Y.Z`. The current product line starts at `ghostftp-v1.0.0` and advances sequentially through patch releases such as `ghostftp-v1.0.1`, `ghostftp-v1.0.2` and `ghostftp-v1.0.3`.
...
Ghost FTP starts at **1.0.0** and follows Semantic Versioning. Patch releases advance sequentially (`1.0.0` → `1.0.1` → `1.0.2` → `1.0.3`), backward-compatible feature releases advance the minor version, and breaking compatibility requires a major-version change.
