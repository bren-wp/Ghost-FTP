...
func TestValidAskpassInvocation(t *testing.T) {
	...
	if !appstart.ValidAskpassInvocation(exe, exe, "force", token) {
	...
	for _, tc := range []struct {
		...
		if appstart.ValidAskpassInvocation(exe, tc.askExe, tc.require, tc.token) {
...
func TestSelectAskpassSecretOnlyUsesRecognizedPrompts(t *testing.T) {
	...
	secret, ok := appstart.SelectAskpassSecret("user@example.test's password:", password, passphrase)
	if !ok || !bytes.Equal(secret, password) {
		t.Fatalf("password prompt did not select password: ok=%v secret=%q", ok, secret)
	}
	secret, ok = appstart.SelectAskpassSecret("Enter passphrase for key 'id_ed25519':", password, passphrase)
	...
	for _, prompt := range []string{
		...
		if secret, ok := appstart.SelectAskpassSecret(prompt, password, passphrase); ok || secret != nil {
...
func TestSelectAskpassSecretRequiresMatchingCredential(t *testing.T) {
	if _, ok := appstart.SelectAskpassSecret("Password:", nil, []byte("passphrase")); ok {
		t.Fatal("password prompt must not fall back to passphrase")
	}
	if _, ok := appstart.SelectAskpassSecret("Passphrase:", []byte("password"), nil); ok {
